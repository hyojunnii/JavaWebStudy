...
import com.kh.app14.board.vo.BoardVo;
...
public class BoardController {
	 ...
	 * 1. 라이브러리 등록
	 * 2. bean 등록
	 * 3. enctype, post 등 통신방식 관련 설정
	 * 
	 * <주의사항>
	 * - form 태그의 method 는 post 로 작성
	 * - form 태그의 enctype 확인
	 * - 컨트롤러쪽 PostMapping 확인
	 * - bean 등록시, id 확인
	 * 
	 * - 파일 객체 받을 때 : MultipartFile
	 * - 파일 비었는지 확인 : f.isEmpty();
	 * - 여러개 파일 받을 때 : MultipartFile[]
	 * - 파일 비었는지 확인 (여러개) :  fArr[0].isEmpty();
	 */
	//게시글작성
	@PostMapping("write")
	public String write(BoardVo vo, HttpServletRequest req) {
		...
		// <파일 저장>
		// 1. 파일이 있는지 확인
		if(!fArr[0].isEmpty()) {				// 클라이언트로부터 전달받은 파일 있음
